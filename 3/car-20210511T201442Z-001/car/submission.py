...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for i in range (self.belief.numRows):
            for j in range(self.belief.numCols):
                prior = self.belief.getProb(i, j)
                dist = math.sqrt((agentY - util.rowToY(i)) ** 2 + (agentX - util.colToX(j)) ** 2)
                emission = util.pdf(dist, Const.SONAR_STD, observedDist)
                posterior = prior * emission
                self.belief.setProb(i, j, posterior)
    ...
    def elapseTime(self):
        ...
        # BEGIN_YOUR_CODE (our solution is 6 lines of code, but don't worry if you deviate from this)
        temp = util.Belief(self.belief.numRows, self.belief.numCols, 0)
        for oldTile, newTile in self.transProb:
            transition_prob = self.transProb[(oldTile, newTile)]
            post_belief = self.belief.getProb(oldTile[0], oldTile[1])
            temp.addProb(newTile[0], newTile[1], post_belief * transition_prob)

        for i in range(self.belief.numRows):
            for j in range(self.belief.numCols):
                self.belief.setProb(i, j, temp.getProb(i, j))
        self.belief.normalize()
        # END_YOUR_CODE
